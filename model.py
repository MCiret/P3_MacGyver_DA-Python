...
class Model:
    MAZE_HALLWAYS_INDEX_DICT = {"all_hallways": [], "guardian_tile": -1, "macgyver_start_tile": -1}
    PICTURES_DICT = {}
    ...
    def initialize_pygame_window():
        ...
        :return: (pygame.Surface) An initialized pygame main window : size, title and icon
        """
        window = pygame.display.set_mode((cste.WINDOW_WIDTH, cste.WINDOW_HEIGHT))
    ...
    def load_pictures(cls):
        """
        Filled the class attribute PICTURE_DICT (dict) with pictures loaded with pygame.
        Each picture is a Tile() instance attribute.
        """
        if len(Model.PICTURES_DICT) == 0:
            for char_key in cste.ASCII_TO_PICTURE_DICT:
                cls.PICTURES_DICT[char_key] = pygame.image.load(cste.ASCII_TO_PICTURE_DICT[char_key]).convert()

    @staticmethod
    def maze_load_from_file(empty_maze_list):
        """
        Each ascii characters representing the maze structure is converted to Tile's subclasses instance using the
        globals() dict (all Tile's subclasses have to be imported here).
        The :param empty_maze_list: an empty list which will be filled and so modified (side effect)
        """
        ...
        with open(cste.MAZE_LVL1, encoding="utf-8") as f:
            for line in f:
                line = line.strip('\n')
                # In the loaded text file, each maze tile is a doubled ascii character.
    ...
    def hallway_index_dict(cls, built_maze_list):
        """
        Fills the class attribute MAZE_HALLWAYS_INDEX_DICT (dict) with indexes (int) of hallways tiles in the maze
        including Guardian and MacGyver indexes which are also recorded apart using 2 more keys => their tiles have not
        to be used for adding the 3 items in the maze.
        :param built_maze_list: (list) maze tiles (Tile() subclasses) filled using the load_from_file() method (above)
        """
    ...
    def rand_items_pos():
        ...
        """
        # Remove Guardian and MacGyver tiles indexes to have only free hallway tiles
        tile_idx_ok_to_place_item = [idx for idx in Model.MAZE_HALLWAYS_INDEX_DICT["all_hallways"]
                                     if idx != Model.MAZE_HALLWAYS_INDEX_DICT["guardian_tile"]
                                     and idx != Model.MAZE_HALLWAYS_INDEX_DICT["macgyver_start_tile"]]
        return sample(tile_idx_ok_to_place_item, k=3)  # random method without replacement

    @staticmethod
    def add_items(rand_items_pos, built_maze_list):
        """
        Item subclasses (Ether, Tube, Needle) are instanced and randomly added in the maze list
        :param built_maze_list: (list) maze tiles (Tile subclasses)
        :param rand_items_pos: (list) indexes (int) to add the 3 items in the maze list
        """
    ...
    def is_wall(maze_tile_id):
        ...
        return maze_tile_id not in Model.MAZE_HALLWAYS_INDEX_DICT["all_hallways"]

    @ staticmethod
    def is_out_of_maze(move_cmd, mg_current_pos_id):
        """
        To avoid moving Mac Gyver out of the maze window.
        :param move_cmd: (int) Number will be added/subtracted to the current MG index to move him in the maze list
        :param mg_current_pos_id: (int) the current MG index in the maze list
        :return: (bool) True if MG is on one of the 4 side and if the move_cmd goes outside
        """
        assert(type(move_cmd) is int)
        assert(type(mg_current_pos_id) is int)

        if move_cmd == -15:  # north move <=> -15 squares
            return mg_current_pos_id < (cste.NB_SPRITE_SIDE - 1)  # MG is on the northern edge of the maze
        elif move_cmd == 15:  # south move <=> +15 squares
            return mg_current_pos_id > (cste.NB_SPRITE_SIDE ** 2) - 1 - cste.NB_SPRITE_SIDE  # MG is on the southern edge of the maze
    ...
    def how_many_items_in_maze(built_maze_list):
        ...
        res_nb_items = 0
        for item in (item_class for item_class in (globals()[cste.ASCII_TO_CLASS_DICT[item_char]]()
